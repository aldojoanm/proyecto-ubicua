...
public class AssignPermissionsToRoleOrchestrator {
    ...
    private static final Logger log = LoggerFactory.getLogger(AssignPermissionsToRoleOrchestrator.class);

    private final AssignPermissionsToRoleUseCase assignPermissionsToRoleUseCase;

    public AssignPermissionsToRoleOrchestrator(AssignPermissionsToRoleUseCase assignPermissionsToRoleUseCase) {
    ...
    public AssignPermissionsToRoleResponse execute(AssignPermissionsToRoleRequest request) {
        log.info("event=IAM_ASSIGN_PERMISSIONS_REQUEST_RECEIVED roleId={} permissionIdsCount={}", 
                request.roleId(), request.permissionIds() != null ? request.permissionIds().size() : 0);

        try {
            AssignPermissionsToRoleCommand command = new AssignPermissionsToRoleCommand(
                    request.roleId(),
            ...
            log.error("event=IAM_ASSIGN_PERMISSIONS_ERROR errorCode={} roleId={}", 
                    e.getCode(), request.roleId(), e);
            throw e;
        } catch (Exception e) {
