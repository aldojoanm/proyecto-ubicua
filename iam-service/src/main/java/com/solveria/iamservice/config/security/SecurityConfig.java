 ...
 * 1. JWT-enabled mode (security.jwt.enabled=true): Requires JWT authentication for /api/**
 * 2. JWT-disabled mode (security.jwt.enabled=false, default): No authentication required (DEV mode)
 * 
 * Both modes allow public access to:
...
public class SecurityConfig {
    ...
    /**
     * Security filter chain for JWT-enabled mode.
     * Requires JWT authentication for /api/** endpoints.
     */
    @Bean
    ...
    public SecurityFilterChain jwtSecurityFilterChain(HttpSecurity http) throws Exception {
        ...
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        // Protected endpoint: Prometheus requires JWT + SCOPE_operations.read
                ...
                // JWT decoder will be auto-configured from spring.security.oauth2.resourceserver.jwt.issuer-uri
    ...
    /**
     * Security filter chain for JWT-disabled mode (DEV mode).
    ...
    /**
     * AuthenticationEntryPoint for 401 Unauthorized responses.
     * Returns ApiErrorResponse with errorCode="UNAUTHORIZED".
     */
    private AuthenticationEntryPoint authenticationEntryPoint() {
        return (request, response, authException) -> {
            log.warn("event=SECURITY_AUTHENTICATION_FAILED path={}", request.getRequestURI(), authException);
            writeErrorResponse(response, request, ErrorCodes.UNAUTHORIZED, HttpServletResponse.SC_UNAUTHORIZED);
        };
    }

    /**
     * AccessDeniedHandler for 403 Forbidden responses.
     * Returns ApiErrorResponse with errorCode="FORBIDDEN".
     */
    private AccessDeniedHandler accessDeniedHandler() {
        return (request, response, accessDeniedException) -> {
            log.warn("event=SECURITY_ACCESS_DENIED path={}", request.getRequestURI(), accessDeniedException);
            writeErrorResponse(response, request, ErrorCodes.FORBIDDEN, HttpServletResponse.SC_FORBIDDEN);
        };
    }
    ...
    private void writeErrorResponse(HttpServletResponse response, HttpServletRequest request, 
                                   String errorCode, int httpStatus) throws IOException {
